(() => {
  ...
  const PATCHES_URL = 'http://netpropatch.tisindia.net/';
  const products = [{"id": "tee-white", "name": "Essential White Tee", "category": "clothing", "color": "White", "swatch": "#f0f0eb", "sizes": ["S", "M", "L", "XL"], "art": "catalog-0", "isNew": true, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "tee-navy", "name": "Essential Navy Tee", "category": "clothing", "color": "Navy", "swatch": "#1d304c", "sizes": ["S", "M", "L", "XL"], "art": "catalog-1", "isNew": false, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "sweatshirt", "name": "Classic Crewneck", "category": "clothing", "color": "Oatmeal", "swatch": "#d9ceba", "sizes": ["S", "M", "L", "XL"], "art": "catalog-2", "isNew": true, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "hoodie", "name": "Basic Hoodie", "category": "clothing", "color": "Sky blue", "swatch": "#a4c6df", "sizes": ["S", "M", "L", "XL"], "art": "catalog-3", "isNew": false, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "socks", "name": "Basic Crew Socks", "category": "socks", "color": "White", "swatch": "#f0f0eb", "sizes": ["S/M", "L/XL"], "art": "catalog-4", "isNew": true, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "socks-black", "name": "Black Crew Socks", "category": "socks", "color": "Black", "swatch": "#252525", "sizes": ["S/M", "L/XL"], "art": "catalog-5", "isNew": false, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "cap", "name": "Basic Blue Cap", "category": "headwear", "color": "Cobalt blue", "swatch": "#285cad", "sizes": ["One size"], "art": "catalog-6", "isNew": false, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "cap-beige", "name": "Basic Beige Cap", "category": "headwear", "color": "Beige", "swatch": "#d6c5a8", "sizes": ["One size"], "art": "catalog-7", "isNew": true, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}, {"id": "patch", "name": "Blank Shape Patches", "category": "patches", "color": "Mixed blues", "swatch": "#3d608e", "sizes": ["One size"], "art": "catalog-8", "isNew": false, "description": "Choose the standard style for direct purchase, or request a custom version for your brand, team or project. Bulk and customization pricing is quoted separately."}];
  products.forEach(p=>p.price={"tee-white": 24, "tee-navy": 24, "sweatshirt": 48, "hoodie": 68, "socks": 12, "socks-black": 12, "cap": 32, "cap-beige": 32, "patch": 8}[p.id]);
  const isShop = document.body.classList.contains("shop-page");
  const shopState = {color:"all",size:[],price:[],sort:"featured",page:1};
  ...
  $$('.patch-link').forEach(button => {
    ...
    a.setAttribute('aria-label', 'Explore Netpro Patches (opens in a new tab)');
  ...
  function renderProducts() {
    ...
    if (isShop) {
      items = items.filter(p => (shopState.color === 'all' || p.color === shopState.color) && (!shopState.size.length || shopState.size.some(size=>p.sizes.includes(size))) && (!shopState.price.length || shopState.price.some(range=>p.price>=Number(range) && p.price<({0:10,10:50,50:100,100:200,200:Infinity}[range]))));
      if (shopState.sort === 'low') items.sort((a,b)=>a.price-b.price);
      if (shopState.sort === 'high') items.sort((a,b)=>b.price-a.price);
      ...
      const label={all:'All Products',new:'New Arrivals',clothing:'Clothing',socks:'Socks',headwear:'Headwear',patches:'Patches'}[activeFilter];
    ...
    $('#product-grid').innerHTML = items.map(p => `<article class="product-card" data-product="${p.id}">
      ...
      <div class="product-meta"><span class="new-label">${p.category === 'patches' ? 'PATCHES' : 'BASIC · CUSTOMIZABLE'}</span><h3><button class="product-name" data-detail="${p.id}">${p.name}</button></h3><p class="product-price">${money(p.price)} <small>· Custom quote available</small></p><p class="product-color"><span class="color-dot" style="--swatch:${p.swatch}"></span>${p.color}</p></div>
  ...
  function showProduct(id) {
    ...
    if (!p) return;
    $('#product-detail').innerHTML = `<div class="detail-layout"><div class="detail-gallery"><div class="gallery-thumbs" aria-label="Product images"></div><div class="detail-visual">${art(p)}</div></div><div class="detail-copy"><span class="new-label">BULK & CUSTOM</span><h2>${p.name}</h2><div class="detail-price">${money(p.price)} <small>Standard blank · Per unit · Demo price</small></div><p>${p.description}</p><p class="product-color"><span class="color-dot" style="--swatch:${p.swatch}"></span>${p.color}</p><fieldset class="detail-sizes"><legend>Choose your size</legend><div class="size-options">${p.sizes.map((size,i)=>`<button type="button" data-detail-size="${size}" aria-pressed="${i===0}">${size}</button>`).join('')}</div></fieldset><div class="detail-quantity"><span>Quantity</span><div><button id="quantity-less" aria-label="Decrease quantity">−</button><input id="detail-quantity" type="number" min="1" max="100000" value="1" aria-label="Requested quantity"><button id="quantity-more" aria-label="Increase quantity">+</button></div></div><button class="button primary buy-standard" data-buy="${p.id}">Add to cart ${arrow}</button><button id="add-to-bag" class="button outline">Request customization ${arrow}</button><div class="price-explanation"><strong>Standard purchase</strong><p>Shown price is for one blank item. Decoration, shipping and taxes are excluded.</p><strong>Bulk & custom pricing</strong><p>Quoted separately by quantity, artwork and finish. Request customization for a project quote.</p></div></div></div>`;
  ...
  const info = {
    ...
    sizes: ['Find your everyday fit', 'Product-specific measurements will be added with the final collection. The current product sizes are preview options, not a confirmed size guide.'],
    ...
    privacy: ['Privacy Policy', 'This standalone preview has no account registration, analytics or payment processing. If you use the preview bag, its contents are stored in your browser on this device. The final store’s privacy policy will be published before launch.'],
    terms: ['Terms of Service', 'This page is a design preview. Product images, prices and options are illustrative. No orders or payments are accepted through this preview. Final store terms will be published before launch.']
  };
