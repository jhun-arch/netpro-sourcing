(() => {
  ...
  function showProduct(id) {
    ...
    if (!p) return;
    $('#product-detail').innerHTML = `<div class="detail-layout"><div class="detail-gallery"><div class="gallery-thumbs" aria-label="Product images"></div><div class="detail-visual">${art(p)}</div></div><div class="detail-copy"><span class="new-label">BULK & CUSTOM</span><h2>${p.name}</h2><div class="detail-price">${money(p.price)} <small>Standard style · Preview price</small></div><p>${p.description}</p><p class="product-color"><span class="color-dot" style="--swatch:${p.swatch}"></span>${p.color}</p><fieldset class="detail-sizes"><legend>Choose your size</legend><div class="size-options">${p.sizes.map((size,i)=>`<button type="button" data-detail-size="${size}" aria-pressed="${i===0}">${size}</button>`).join('')}</div></fieldset><div class="detail-quantity"><span>Quantity</span><div><button id="quantity-less" aria-label="Decrease quantity">−</button><input id="detail-quantity" type="number" min="1" max="100000" value="1" aria-label="Requested quantity"><button id="quantity-more" aria-label="Increase quantity">+</button></div></div><button class="button primary buy-standard" data-buy="${p.id}">Add to cart ${arrow}</button><button id="add-to-bag" class="button outline">Request customization ${arrow}</button><p class="detail-note">MOQ, material specifications, sample availability and lead time are confirmed for each project.</p></div></div>`;
